...
def navbar_component():
    ...
    for key, value in NAVBAR_PATHS.items():
        print(key)
        if key == "LooLearn.ai":
            navbar_items += (f'<a class="mainnavitem" href="/?nav={value}">{key}</a>')
        else:
    ...
    <script>
        ...
        var navigationTabs_header = window.parent.document.getElementsByClassName("mainnavitem");
        ...
        for (var i = 0; i < navigationTabs.length; i++) {
         ...
         for (var i = 0; i < navigationTabs_header.length; i++) {
            cleanNavbar(navigationTabs_header[i]);
        }
